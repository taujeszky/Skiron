...
Written 2026-09-19, when nothing was built and the repository held this plan, a README,
a CLAUDE.md and a .gitignore. **It is kept as written**, so that what it got wrong stays
visible; every correction the code forced is recorded in the wave file it belongs to and
in `docs/ARCHITECTURE.md`, never by editing the design out from under itself. The status
table at the bottom says where things actually are.
...
| 8 | `wave-8-ship.md` | Tutorial, polish, docs, repo, deploy, catalog | **done** 2026-09-21 — public at [github.com/taujeszky/Skiron](https://github.com/taujeszky/Skiron), live at [skiron-e0f.pages.dev](https://skiron-e0f.pages.dev), twentieth card in the catalog. Tasks 1-7 gave two tutorial lessons, the balance pass re-measured and deliberately unchanged, accessibility audited with `npm run contrast` as a permanent guard, case files that are re-proved on import, and three bugs that had been shipping: a pack case could not be resumed, the writing path printed the provider's raw error, and the floor plan's rooms were announced as nothing. Task 8 was offered and held on the morning of 2026-09-21 and asked for the same day |
